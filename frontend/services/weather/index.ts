...
export async function fetchWeather(city = "Paris", lat?: number, lon?: number): Promise<WeatherContext> {
  ...
  if (!apiKey || apiKey.trim() === "") {
    ...
    return getMockWeather(mockCityName);
  }
  ...
  try {
    ...
    const response = await fetch(url, { next: { revalidate: 900 } }); // Cache for 15 mins
    if (!response.ok) {
    ...
    try {
      ...
      const response = await fetch(fallbackUrl, { next: { revalidate: 900 } });
      if (response.ok) {
        const data = await response.json();
